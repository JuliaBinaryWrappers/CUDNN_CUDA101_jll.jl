...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import CUDNN_CUDA101_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("CUDNN_CUDA101_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "CUDNN_CUDA101_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"CUDNN_CUDA101", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "CUDNN_CUDA101_jll", "src", "CUDNN_CUDA101_jll.jl"))
        @info("CUDNN_CUDA101_ll dev'ed out to /depot/dev/CUDNN_CUDA101_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("0e66bb0f-9c4c-572f-9142-addab54c8167"))
platforms = [Pkg.Artifacts.unpack_platform(e, "CUDNN_CUDA101", artifacts_toml) for e in artifacts["CUDNN_CUDA101"]]
...
if best_platform === nothing
    @debug("Unable to load CUDNN_CUDA101; unsupported platform $(triplet(platform_key_abi()))")
else
